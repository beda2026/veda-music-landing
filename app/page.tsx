    ...
    <>
      <header className="mx-auto w-full max-w-[1400px] px-4 pt-6 sm:px-6 lg:px-10">
        <section className="ad-box ad-space-labels mb-4 rounded-2xl p-4 sm:p-5 lg:min-h-[132px] lg:px-6">
          <div className="flex items-center justify-center overflow-hidden rounded-xl border border-zinc-600/70 bg-black/30 backdrop-blur-xl p-2 sm:p-3">
            <img
      ...
      <main className="mx-auto flex w-full max-w-[1400px] flex-col gap-8 px-4 py-8 sm:px-6 lg:px-10">
        <section className="grid gap-4 lg:grid-cols-[minmax(0,65fr)_minmax(0,35fr)] lg:items-stretch">
          ...
          <aside className="ad-box ad-space-labels flex min-h-[280px] flex-col items-center justify-center rounded-3xl p-4 text-center lg:h-[300px] lg:min-h-[300px]">
            <div
              className="relative block h-[250px] w-full max-w-[336px] overflow-hidden rounded-2xl border border-zinc-600/80 bg-black/30 backdrop-blur-xl"
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            {vedaArtists.map((artist) => (
              <article key={artist.slug} className="panel hover-card rounded-2xl p-4 media-card">
                <div className="mb-3 h-36 overflow-hidden rounded-xl border border-zinc-700 bg-gradient-to-br from-black/45 via-red-950/8 to-yellow-950/6 backdrop-blur-xl">
                  {artist.image ? (
                    <img src={artist.image} alt={artist.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full items-end p-3">
                      <span className="rounded-full border border-[#f5b21b]/60 bg-black/30 px-2 py-1 text-[10px] font-semibold tracking-[0.12em] text-[#f5b21b]">V.E.D.A. ARTIST</span>
                    </div>
                  )}
                </div>
          ...
          <div className="panel rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {releases.map((release) => (
                <article key={release} className="hover-card rounded-xl border border-white/10 bg-black/30 backdrop-blur-xl p-3 before:mr-2 before:text-[#f5b21b] before:content-['●']">
                  {release}
                </article>
              ))}
            </div>
          ...
          <div className="promo-packages rounded-2xl p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {promoPackages.map((pack) => (
                <div key={pack} className="rounded-xl border border-white/10 bg-black/30 backdrop-blur-xl px-4 py-3 text-sm text-zinc-200">
                  {pack}
        ...
        <section className="ad-box rounded-3xl border border-[#f5b21b]/25 bg-gradient-to-br from-black/45 via-red-950/8 to-yellow-950/6 backdrop-blur-xl p-6 md:p-7">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {brandPartners.map((partner) => (
              <article
                key={partner.name}
                className="group rounded-2xl border border-zinc-700/80 bg-black/30 p-4 shadow-[0_8px_28px_rgba(0,0,0,0.35)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-[#f5b21b]/55 hover:shadow-[0_12px_30px_rgba(245,178,27,0.12)]"
              >
                <div className="relative h-[190px] overflow-hidden rounded-xl border border-white/10 bg-black/30 backdrop-blur-xl flex items-center justify-center">
                  <img
                    src={partner.image}
          ...
          <div className="mt-6 rounded-2xl border border-[#f5b21b]/35 bg-black/30 p-5 text-center shadow-[0_8px_28px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      ...
      <footer className="border-t border-zinc-800 bg-black/30 backdrop-blur-xl">
