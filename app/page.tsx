    ...
    <>
      <header className="mx-auto w-full max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <div className="panel mb-4 flex flex-col gap-3 rounded-2xl p-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <p className="text-xs uppercase tracking-[0.2em] text-zinc-400">SÍGUENOS: f · X · IG · YT · TT · SP</p>
        </div>
        ...
        <div className="panel flex flex-col gap-4 rounded-2xl p-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <nav className="flex flex-wrap gap-3 text-sm font-medium text-zinc-200">
            {navLinks.map((link) => (
              <a key={link} href={link === 'Inicio' ? '#' : `#${link.toLowerCase()}`} className="rounded-full border border-zinc-700 px-3 py-1.5 transition hover:border-[#ef1f2d] hover:text-white">
                {link}
              </a>
            ))}
          </nav>
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        <section className="grid gap-4 lg:grid-cols-[1.1fr_1fr_300px]">
          <article className="panel rounded-3xl p-6">
            <p className="mb-3 text-xs uppercase tracking-[0.22em] text-zinc-400">V.E.D.A. MUSIC</p>
            <h1 className="text-4xl font-black leading-tight text-zinc-50 lg:text-5xl">LA NUEVA PLATAFORMA URBANA</h1>
            <p className="mt-4 max-w-md text-zinc-300">Videos, entrevistas, estrenos, fotos y todo lo que mueve la cultura.</p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#videos" className="btn-red">Ver lo más nuevo</a>
              <a href="#entrevistas" className="btn-gold">Explorar artistas</a>
            </div>
            <div className="mt-6 h-28 rounded-2xl border border-zinc-700 bg-[linear-gradient(120deg,#ef1f2d20,#f5b21b20,#111)]" />
          </article>

          <article className="panel hover-card rounded-3xl p-5">
            <span className="inline-block rounded-full border border-[#f5b21b] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[#f5b21b]">ENTREVISTA / ESTRENO</span>
            <div className="my-4 flex h-44 items-center justify-center rounded-2xl border border-zinc-700 bg-[radial-gradient(circle_at_top,#ef1f2d44,transparent_45%),linear-gradient(140deg,#111,#1b1b1b)]">
              <span className="text-6xl font-black text-zinc-500">V</span>
            </div>
        ...
        <section id="videos" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {videos.map((video) => (
              <article key={video.title} className="panel hover-card rounded-2xl p-4">
                <div className="mb-3 flex h-40 items-center justify-center rounded-xl border border-zinc-700 bg-[linear-gradient(145deg,#ef1f2d24,#111)] text-2xl">▶</div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {photos.map((photo) => (
              <article key={photo.title} className="panel hover-card rounded-2xl p-4">
                <div className="mb-3 h-36 rounded-xl border border-zinc-700 bg-[radial-gradient(circle,#f5b21b22,transparent_60%),linear-gradient(130deg,#191919,#101010)]" />
        ...
        <section id="entrevistas" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {interviews.map((item) => (
              <article key={item.title} className="panel hover-card rounded-2xl p-4">
          ...
          <div className="panel rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {releases.map((release) => (
                <article key={release} className="hover-card rounded-xl border border-zinc-700 bg-[#121212] p-3">
                  {release}
                </article>
              ))}
            </div>
        ...
        <section id="contacto" className="space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {services.map((service) => (
              <article key={service.title} className="panel hover-card rounded-2xl p-4">
        ...
        <section className="ad-box rounded-2xl p-5 text-center">
          ...
          <div className="grid gap-3 sm:grid-cols-5">
            {Array.from({ length: 5 }).map((_, idx) => (
              <div key={`slot-${idx}`} className="rounded-xl border border-zinc-700 bg-[#151515] p-4 text-sm text-zinc-400">Tu logo aquí</div>
            ))}
          </div>
        </section>
      </main>
